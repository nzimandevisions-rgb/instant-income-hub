...
// Live Feed Configuration (White-labeled)
...
export function HomePage() {
  ...
  // Interactive Live Offer Popup Modal
  ...
  const loadLiveOffers = async (isManual = false) => {
    ...
    try {
      // 1. Primary auto-detected geo feed
      ...
      // 2. Fallback to South Africa & Global inventory so offers always appear
      ...
      // 3. Map into SydeHustle clean structure
      const normalized: TaskOffer[] = rawList.map((item: any, idx: number) => {
        ...
        return {
          ...
          description: item.description || "Follow instructions on the target page to earn instant points.",
          payout: rawPayout,
          points,
          usdValue: usd,
          zarValue: zar,
      ...
      toast.error("Could not load tasks. Retrying...");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">
              Complete Tasks, Earn Real Cash
            </h1>
            <p className="text-sm text-muted-foreground max-w-xl mt-1.5">
              Choose an offer below. Earn points and cash out instantly to <strong>PayPal</strong>, <strong>MTN MoMo</strong>, <strong>M-Pesa</strong>, or <strong>Airtime</strong>.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              to="/wallet"
              className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold bg-emerald-500 text-slate-950 hover:bg-emerald-400 transition-colors shadow-md"
            >
              <Wallet className="w-4 h-4" />
              Cash Out (PayPal / MoMo)
            </Link>
            <button
              type="button"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-6 pt-6 border-t border-border/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-sm font-bold text-foreground">{offers.length} Live</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 col-span-2 sm:col-span-1">
            ...
            <div>
              <span className="text-xs text-muted-foreground block">Payouts</span>
              <span className="text-sm font-bold text-foreground">PayPal & Mobile Money</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 px-4 bg-card rounded-2xl border border-border">
          ...
          <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1 mb-4">
            Try resetting your search or click below to reload all regional tasks.
          </p>
          <button
            type="button"
            onClick={() => {
          ...
          >
            ...
            <div className="space-y-2.5 text-xs text-muted-foreground">
              ...
              <div className="space-y-2 bg-muted/40 p-3.5 rounded-xl border border-border">
                ...
                <div className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                  <span>2. Follow the prompt instructions on the target page.</span>
                </div>
                <div className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />

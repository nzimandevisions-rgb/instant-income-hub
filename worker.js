...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/public/postback") {
      ...
      if (!env.DB) return new Response("D1 database binding missing", { status: 500 });
      ...
      try {
        await env.DB.prepare("INSERT OR IGNORE INTO users (id, email, points) VALUES (?, ?, 0)").bind(subId, subId + "@user.sydehustle.com").run();
        const transactionId = ("txn_" + subId + "_" + txid).slice(0, 220);
        const inserted = await env.DB.prepare("INSERT OR IGNORE INTO transactions (id, user_id, amount, txid, type) VALUES (?, ?, ?, ?, ?)").bind(transactionId, subId, reward, txid, "job_reward").run();
        if (inserted.meta?.changes !== 0) await env.DB.prepare("UPDATE users SET points = points + ? WHERE id = ?").bind(reward, subId).run();
    ...
    if (url.pathname === "/api/user/withdraw" && request.method === "POST") {
      ...
      try {
        const debited = await env.DB.prepare("UPDATE users SET points = points - ? WHERE id = ? AND points >= ?").bind(points, userId, points).run();
        ...
        await env.DB.prepare("INSERT OR IGNORE INTO transactions (id,user_id,amount,txid,type) VALUES (?,?,?,?,?)").bind(reference, userId, points, reference, "withdrawal").run();
        const balance = await env.DB.prepare("SELECT points FROM users WHERE id = ?").bind(userId).first();
        return Response.json({ success: true, reference, balance: balance?.points || 0, provider });
      } catch (error) {
        try { await env.DB.prepare("UPDATE users SET points = points + ? WHERE id = ?").bind(points, userId).run(); } catch {}
        return Response.json({ error: error instanceof Error ? error.message : "Payout failed" }, { status: 502 });
      }
    }

    // Static assets first. If a request is not a static asset, let TanStack Start
    // render the application/route server-side.
    if (env.ASSETS) {
